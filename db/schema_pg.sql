...
-- Stage 3: Event ledger + daily score snapshots
ALTER TABLE mechanism_rules ADD COLUMN IF NOT EXISTS half_life_days REAL DEFAULT 90;
ALTER TABLE analysis ADD COLUMN IF NOT EXISTS event_processed BOOLEAN DEFAULT FALSE;

UPDATE mechanism_rules SET half_life_days = 120 WHERE mechanism_type = 'rate_high' AND (half_life_days IS NULL OR half_life_days = 90);
UPDATE mechanism_rules SET half_life_days = 120 WHERE mechanism_type = 'rate_falling' AND (half_life_days IS NULL OR half_life_days = 90);
UPDATE mechanism_rules SET half_life_days = 60  WHERE mechanism_type = 'supply_chain_cost_rise' AND (half_life_days IS NULL OR half_life_days = 90);
UPDATE mechanism_rules SET half_life_days = 180 WHERE mechanism_type = 'ai_capex_rising' AND (half_life_days IS NULL OR half_life_days = 90);
UPDATE mechanism_rules SET half_life_days = 45  WHERE mechanism_type = 'usd_strengthening' AND (half_life_days IS NULL OR half_life_days = 90);
...
-- technical_score: volume-confirmed support/resistance indicator (added 2026-06-12)
ALTER TABLE daily_score_snapshot ADD COLUMN IF NOT EXISTS technical_score REAL;

CREATE INDEX IF NOT EXISTS idx_event_ledger_ticker_date ON event_ledger(ticker, event_date);
CREATE INDEX IF NOT EXISTS idx_event_ledger_signal_id ON event_ledger(signal_id);
CREATE INDEX IF NOT EXISTS idx_daily_snapshot_ticker_date ON daily_score_snapshot(ticker, snapshot_date);
